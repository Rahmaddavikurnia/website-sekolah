...
<div class="page-wrapper">
    ...
    <div class="blog-classic-section">
    	<div class="auto-container">
            ...
            @foreach ($humas as $item)
                        	<!--News Block-->
        	<div class="news-block-eight">
        		<div class="inner-box">
        			<div class="image">
						<a href="{{route('detail-humas',$item->slug)}}"><img src="{{asset('storage/images/humas/'.$item->gambar)}}" alt="img" /></a>
					</div>
					<div class="content">
						<div class="clearfix">
							<div class="pull-left">
								<ul class="post-meta clearfix">
									<li>{{$item->tanggal_kegiatan}}</li>
									<li>{{$item->pihak_terkait}}</li>
						...
						<h2><a href="{{route('detail-humas',$item->slug)}}">{{$item->judul_kegiatan}}</a></h2>
						<div class="text">{{ Str::limit($item->deskripsi_kegiatan, 300, '...') }}</div>
						<a class="read-more" href="{{route('detail-humas',$item->slug)}}">Lihat Selengkapnya</a>
					</div>
				</div>
			</div>
			...
			<!--Styled Pagination-->
            {{ $humas->links('frontend.pagination') }}
